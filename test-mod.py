...
if __name__ == "__main__":
    ...
    for img_idx in range(len(testset)):
        ...
        label = label.flatten()
        patches_pred.append(label_pred)
    ...
    print(np.max(patches_pred))
    # reodenar los patches
    for a in patches_pred:
        plt.imshow(a, cmap="jet", vmin=0, vmax=np.max(patches_pred))
        plt.show()
        plt.clf()
