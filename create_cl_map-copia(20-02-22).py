...
if __name__ == "__main__":
    ...
    clmap = np.zeros(((1920, 3968, 3)))

    for a,b in [(0,1728), (1728,2240), (2240,-1)]:
        rgb = image_rgb[:,a:b,:]
        rad = image_hsi_rad[:,a:b,:]
        ref = image_hsi_ref[:,a:b,:]
        labels = image_labels[:,a:b,:]
        ...
        for xx in range(0, x_arr - size_chips//2, size_chips//2):
            for yy in range(0, y_arr - size_chips//2, size_chips//2):
                ...
                if shapeim[0] == shapeim[1] and shapeim[0] == size_chips:                    
                    ...
                    if clmap[a+xx:a+xx+size_chips,a+yy:a+yy+size_chips].shape[0] == \
                        clmap[a+xx:a+xx+size_chips,a+yy:a+yy+size_chips].shape[1] and \
                        clmap[a+xx:a+xx+size_chips,a+yy:a+yy+size_chips].shape[0] == size_chips and \
                        clmap[a+xx:a+xx+size_chips,a+yy:a+yy+size_chips].shape[1] == size_chips:
                            clmap[a+xx:a+xx+size_chips,a+yy:a+yy+size_chips, :] = labels_temp
#                if xx > 340:
#                    salir=True
#                    break
            print(xx)
#            if salir: break
#        break
    sio.savemat("clmap_pred.mat", {"clmap_pred":clmap})
    plt.imshow(clmap, vmin=0)
    plt.show()
